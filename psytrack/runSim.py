...
def generateSim(K=4,
    ...
    if gaussian:
        ...
        for i in range(iterations):
            sim_y = np.sum(X * W, axis=1) + np.random.normal(
                scale=sigmay, size=N)
            all_simy += [sim_y]
...
def recoverSim(data, N=None, iteration=0, hess_calc="All", save=False,gaussian=False):
    ...
    hyp, evd, wMode, hess_info = hyperOpt(dat, hyper_guess, weights, optList,
                                          hess_calc=hess_calc,gaussian=gaussian,showOpt=0)
    END = datetime.now()
